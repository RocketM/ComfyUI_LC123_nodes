...
class AspectRatioSimplifier:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "custom_width": (
                    "INT",
                    {"default": 1024, "min": 16, "max": MAX_RESOLUTION, "step": 8, "tooltip": "Target width for custom size."},
                ),
                "custom_height": (
                    "INT",
                    {"default": 1024, "min": 16, "max": MAX_RESOLUTION, "step": 8, "tooltip": "Target height for custom size."},
                ),
                "swap_dimensions": (["Off", "On"], {"tooltip": "Swap width and height after size is chosen."}),
                "upscale_method": (UPSCALE_METHODS, {"default": "lanczos", "tooltip": "Resampling filter. Lanczos is a good default for photos."}),
                "proportion": (PROPORTIONS, {"default": "crop", "tooltip": "stretch=force size; resize=fit inside; crop=fill+crop; pad=fit+letterbox; total_pixels=match pixel budget."}),
                "crop_location": (CROP_LOCATIONS, {"default": "center", "tooltip": "Crop anchor when proportion is crop."}),
                "pad_color": (
                    "STRING",
                    {
                        "default": "0, 0, 0",
                ...
                "divisible_by": (
                    "INT",
                    {"default": 8, "min": 1, "max": 128, "step": 1, "tooltip": "Round width/height to a multiple of this (8 is typical for latents)."},
                ),
                "batch_size": (
                    "INT",
                    {
    ...
    def run(
        ...
        return (out_image, out_mask, int(out_w), int(out_h), latent, int(batch_size), int(max(out_w, out_h)))





class LCAspectRatioPipeOut:
    """Same as Aspect Ratio Simplifier, plus pipe as the top output."""

    @classmethod
    def INPUT_TYPES(cls):
        return AspectRatioSimplifier.INPUT_TYPES()
    ...
    def run(self, **kwargs):
        ...
        return (pipe, image, mask, width, height, latent, batch, resolution)


class LCAspectRatioPipe:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "pipe": ("LC_PIPE", {
                    "tooltip": "Pipe from LC Aspect Ratio Simplifier 📐(Pipe) or LC Pipe.",
                }),
            },
        }
    ...
    def unpack(self, pipe):
        if not isinstance(pipe, dict):
            pipe = {"_type": "LC_PIPE"}
        w = int(pipe["width"]) if pipe.get("width") is not None else 0
        h = int(pipe["height"]) if pipe.get("height") is not None else 0
        res = int(pipe["resolution"]) if pipe.get("resolution") is not None else int(max(w, h))
        return (
            pipe,
            pipe.get("image"),
            pipe.get("mask"),
            ...
            h,
            pipe.get("latent"),
            int(pipe["batch"]) if pipe.get("batch") is not None else 1,
            res,
        )
